...
def show_async_checks():
    ...
    for func in (inspect.iscoroutinefunction, inspect.iscoroutine, inspect.isasyncgenfunction, inspect.isasyncgen, inspect.isawaitable):
        print('==', func.__name__)
...
class ServerProcess:
    ...
    def __enter__(self):
        backend = 'uvicorn'
        self._p = multiprocessing.Process(target=run, args=(self._handler, True, backend))
        self._p.start()
        return self
...
def test_normal_usage():
    ...
    assert set(res.headers.keys()) == {'server', 'date', 'content-type', 'content-length', 'xx-foo'}
